...
def criar_token(id_usuario):
    data_expiracao = datetime.now(timezone.utc) + timedelta(minutes=ACCES_TOKEN_EXPIRES_MINUTES)
    dic_info = {'sub': id_usuario, 'exp': data_expiracao}
    jwt_codificado = jwt.enconde(dic_info, SECRET_KEY, ALGORITHM)
...
async def login(login_schema: LoginSchema, session: Session = Depends(pegar_session)):
    ...
    if not usuario:
        ...
        access_token = criar_token(login_schema.email)
